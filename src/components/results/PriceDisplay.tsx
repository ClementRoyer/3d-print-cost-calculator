  ...
  return (
    <div className="space-y-2">
      ...
      {compact ? (
        <div>
          {selectedPrice === 'selling' ? (
            <div className="rounded-lg shadow-lg p-3 text-white bg-gradient-to-r from-blue-600 to-purple-600 relative">
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-sm font-bold">{t.sellingPrice}</h2>
                <button
                  onClick={onShowTips}
              ...
              <div className="text-xl font-bold">{formatPrice(results.sellingPrice, currency)}</div>
              <p className="text-blue-100 text-sm">{t.perUnit}</p>
            </div>
          ) : (
            <div className="rounded-lg shadow-lg p-3 text-white bg-gradient-to-r from-green-600 to-emerald-600 relative">
              <div className="flex items-center justify-between mb-1">
                <h2 className="text-sm font-bold">{t.recommendedPrice}</h2>
                <button
                  onClick={onShowTips}
              ...
              <div className="text-xl font-bold">{formatPrice(recommendedPrice, currency)}</div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-1.5 px-1">
            {selectedPrice === 'selling' ? t.recommendedPrice : t.sellingPrice}:{' '}
            {formatPrice(selectedPrice === 'selling' ? recommendedPrice : results.sellingPrice, currency)}
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-2 gap-4">
          {/* Selling Price */}
          <div className={`rounded-lg shadow-lg p-4 text-white relative transition-opacity ${
            selectedPrice === 'selling'
